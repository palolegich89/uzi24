...
$rsData = $entity_data_class::getList(array(
   ...
   'order' => array(),
   'limit' => $limit,
   'filter' => array("UF_ACTIVE" => true, "!UF_RAYON" => false) 
));
while($el = $rsData->fetch())
{
	if(!empty($el["UF_RAYON"])){$items[$el["UF_RAYON"]] = $el["UF_RAYON"];}
}

$arSelect = Array("ID", "DATE_ACTIVE_FROM", "CODE");
$arFilter = Array("IBLOCK_ID"=>2, "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", "ID"=>$items);
...
while($ob = $res->GetNextElement())
{
	...
	$sitemap_products .= '	<url>'.PHP_EOL;
		...
		$sitemap_products .= '		<loc>'.SITE_URL.'/location/rayon/'.$arFields["CODE"].'/</loc>'.PHP_EOL;
