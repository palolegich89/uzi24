...
$rsData = $entity_data_class::getList(array(
   ...
   'order' => array(),
   'limit' => $limit,
   'filter' => array("UF_ACTIVE" => true, "!UF_METRO" => false) 
));
while($el = $rsData->fetch())
{
	if(!empty($el["UF_METRO"])){$items[$el["UF_METRO"]] = $el["UF_METRO"];}
}

$arSelect = Array("ID", "DATE_ACTIVE_FROM", "CODE");
$arFilter = Array("IBLOCK_ID"=>1, "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", "ID"=>$items);
...
while($ob = $res->GetNextElement())
{
	...
	$sitemap_products .= '	<url>'.PHP_EOL;
		...
		$sitemap_products .= '		<loc>'.SITE_URL.'/location/metro/'.$arFields["CODE"].'/</loc>'.PHP_EOL;
